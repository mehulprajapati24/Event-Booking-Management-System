...
router.get("/event/:eventName", verifyToken2, (req, res) => {
  ...
  if (emailID) {
    sql =
      "select * from locked_profile lp join user_registration ur on lp.user_id = ur.user_id where ur.email_id=?";
    con.query(sql, [emailID], (err, result) => {
      if (result.length > 0) {
        var eventName = req.params.eventName;
        var sql = `select * from event where eventName=?;`;
        con.query(sql, [eventName], (err, result) => {
          ...
          if (event_date < todayDate) {
            res.render("eventPage2.ejs", {
              locked: "locked",
              closedRegistration: true,
              MaxAttendeesReached: "yes",
              clashesEventsName: "empty",
              register: "Register",
              loggedIn: "yes",
              first_name: firstNAME,
            ...
            con.query(sql, [emailID], (err, result) => {
              ...
              sql =
                "select * from registered_event where user_id=? AND event_id=?";
                          ...
                          function checkEventClash(
                            ...
                            if (event.eventDate === selectedDate) {
                              ...
                              const eventEndTime = moment(
                                event.eventDate + " " + event.eventEndTime,
                                "DD/MM/YYYY h:mm A"
                              );
                              ...
                              if (
                                selectedDateTimeStart.isBetween(
                                  eventStartTime,
                                  eventEndTime
                                ) ||
                                selectedDateTimeEnd.isBetween(
                                  eventStartTime,
                                  eventEndTime
                                ) ||
                                ...
                                clashesEventsName =
                                  clashesEventsName + event.eventName + ", ";
                          ...
                          if (clashFound) {
                            ...
                            if (registrationFees == 0) {
                              ...
                              res.render("eventPage2.ejs", {
                                ...
                                clashWarning:
                                  "Clash found! Are you still want to proceed?",
                                register: "Register",
                                loggedIn: "yes",
                                first_name: firstNAME,
                                email_id: emailID,
  ...
  con.query(sql, [emailID], (err, result) => {
    ...
    con.query(sql, [userId], async (err, result) => {
      if (result.length > 0) {
        ...
        //console.log(name);
        res.render("registeredEvents.ejs", {
          first_name: firstNAME,
          registered: true,
          name: name,
          venue: venue,
