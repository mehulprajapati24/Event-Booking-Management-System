    ...
    var lockedProfiles = [];
      ...
      con.query(sql, (err, result)=>{
        ...
        for(i=0; i<result.length; i++){
          ...
          res.render("adminDashboard.ejs", {lockedProfiles: lockedProfiles, userOriginalID: userOriginalID, userName: userName, userEmail: userEmail, userPhone: userPhone, userGender: userGender, userDOB: userDOB, userDepartment: userDepartment, userYear: userYear, userEnroll: userEnroll});
      });
  });
})
              ...
              if(err){
                res.send(err);
                  ...
                  con.query(sql,[req.body.eventName, req.body.eventTagline, req.body.eventDescription, req.body.eventVenue, req.body.numberOfCoordinators, filename1, filename2, formattedDate, eventStartTime, eventEndTime, req.body.registrationFees, req.body.attendeesCapacity, department, faculty_email],(err,result)=>{
                    ...
                    res.render('adminEvent.ejs', { eventData: req.body.eventName});
                  });
                  });
              }
            });
  ...
  if (key === "value") {
    ...
    con.query(sql, (err, result)=>{
      ...
      con.query(sql, [selectedDepartment], (err, result)=>{
        ...
        res.render("adminEventStatistics.ejs", {departments: departments, selectedDepartment: selectedDepartment, events: events});
      });
    });
    }
  }
...
router.post("/manage_department/edit", (req,res)=>{
  ...
  con.query(sql, (err, result)=>{
    ...
    res.render("editDepartment.ejs", {departmentName:departmentName, departmentIcon:departmentIcon, departmentId:departmentId});
  });
});
...
router.post("/manage_faculty/edit", (req,res)=>{
  ...
  con.query(sql, (err, result)=>{
    ...
    res.render("editFaculty.ejs", {facultyId:facultyId, facultyName:facultyName, facultyEmail:facultyEmail, associatedDepartment:associatedDepartment});
  });
});
...
router.post("/manage_event/edit", (req,res)=>{
  ...
  con.query(sql, (err, result)=>{
    ...
    res.render("editEvent.ejs", {eventId:eventId, eventName:eventName, eventTagline:eventTagline, eventDescription:eventDescription, eventVenue:eventVenue, numberOfCoordinators:numberOfCoordinators, eventIcon:eventIcon, eventFlyer:eventFlyer, eventDate:eventDate, eventStartTime:eventStartTime, eventEndTime:eventEndTime, registrationFees:registrationFees, attendeesCapacity:attendeesCapacity, department:department, facultyEmailId:facultyEmailId});
  });
});
...
router.post("/manage_department/edit/update", (req,res)=>{
  ...
  con.query(sql, [departmentId], (err, result)=>{
    ...
    res.render("updateDepartment.ejs", {departmentName:departmentName, departmentIcon:departmentIcon, departmentId:departmentId});
    });
});
...
router.post("/manage_faculty/edit/update", (req,res)=>{
  ...
  con.query(sql, [facultyId], (err, result)=>{
    ...
    con.query(sql,(err,results)=> {
      ...
      res.render("updateFaculty.ejs", {facultyId:facultyId, facultyName:facultyName, facultyEmail:facultyEmail, associatedDepartment:associatedDepartment, departments: departmentArray});
    });
    });
});
...
router.post("/manage_event/edit/update", (req,res)=>{
  ...
  con.query(sql, [eventId], (err, result)=>{
    ...
    res.render("updateEvent.ejs", {eventId:eventId, eventName:eventName, eventTagline:eventTagline, eventDescription:eventDescription, eventVenue:eventVenue, numberOfCoordinators:numberOfCoordinators, eventIcon:eventIcon, eventFlyer:eventFlyer, eventDate:eventDate, registrationFees:registrationFees, attendeesCapacity:attendeesCapacity, department:department, facultyEmailId:facultyEmailId, hours1:hours1, minutes1:minutes1, period1:period1, hours2:hours2, minutes2:minutes2, period2:period2});
    });
});
      ...
      var destinationPath = path.join(__dirname, '..', 'assets', 'img', 'departments',filename);
        file.mv(destinationPath, (err)=>{
          if(err){
            ...
            console.log('File moved successfully');
            sql = "update department set departmentName=?, departmentIcon=? where id=?";
            con.query(sql,[req.body.departmentName ,filename, departmentId],(err,result)=>{
              if(err) throw err;
...
router.post("/manage_faculty/edit/updatefaculty", (req,res)=>{
  ...
  var associatedDepartment = req.body.departmentName;
      ...
      sql = "update faculty set facultyName=?, facultyEmail=?, facultyPassword=?, associatedDepartment=? where id=?";
  
      bcrypt.hash(facultyPassword, saltRounds, (err,hashPassword)=>{
        con.query(sql,[facultyName ,facultyEmail, hashPassword, associatedDepartment, facultyId],(err,result)=>{
          if(err) throw err;
