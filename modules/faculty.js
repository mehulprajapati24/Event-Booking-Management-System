...
router.get("/", verifyToken,(req,res)=>{
  ...
  var sql = "select distinct ur.* from user_registration ur join registered_event re on ur.user_id = re.user_id join event e on re.event_id = e.id join faculty f on f.associatedDepartment = e.department where f.facultyEmail=?";
    con.query(sql, [req.faculty.facultyEmail], (err, result)=>{
      ...
      con.query(sql, (err, result)=>{
        ...
        for(i=0; i<result.length; i++){
          ...
          res.render("facultyDashboard.ejs", {lockedProfiles:lockedProfiles, userOriginalID: userOriginalID, userName: userName, userEmail: userEmail, userPhone: userPhone, userGender: userGender, userDOB: userDOB, userDepartment: userDepartment, userYear: userYear, userEnroll: userEnroll ,facultyName: req.faculty.facultyName, facultyEmail: req.faculty.facultyEmail});
        });
    });
});
...
router.post("/", (req,res)=>{
    ...
    con.query(sql, faculty_email, (err, result)=>{
      ...
      con.query(sql, (err, result)=>{
        ...
        res.render("facultyDashboard.ejs", {lockedProfiles:lockedProfiles, userOriginalID: userOriginalID, userName: userName, userEmail: userEmail, userPhone: userPhone, userGender: userGender, userDOB: userDOB, userDepartment: userDepartment, userYear: userYear, userEnroll: userEnroll ,facultyName: faculty_name, facultyEmail: faculty_email});
      });
    });
  });
...
router.post("/eventselected",(req,res)=>{
  ...
  con.query(sql, [event_name], (err, result)=>{
    ...
    sql = "SELECT eventName, department FROM event WHERE eventName NOT IN (SELECT associatedEvent FROM coordinator) AND department=?";
    var events = [];
    con.query(sql, [department],(err,result)=>{
        if(err) throw err;
        for(var i=0; i<result.length;i++){
...
router.post("/eventstatistics", verifyToken, (req, res)=>{
  ...
  if (key === "value") {
    ...
    con.query(sql, [req.faculty.facultyEmail], (err, result)=>{
      ...
      res.render("facultyEventStatistics.ejs", {facultyName: req.faculty.facultyName, events:events});
    });
  }
  else{
    ...
    sql = "select u.* from user_registration u join registered_event re on u.user_id = re.user_id join event e on re.event_id = e.id where e.eventName=?";
          con.query(sql, [selectedEvent],(err, result)=>{
            ...
            res.render("facultyEventStatistics.ejs", {facultyName: req.faculty.facultyName, events: events, selectedEvent: selectedEvent, userOriginalID: userOriginalID, userName: userName, userEmail: userEmail, userPhone: userPhone, userGender: userGender, userDOB: userDOB, userDepartment: userDepartment, userYear: userYear, userEnroll: userEnroll});
          });
        });
  }
});
...
router.post("/manage_coordinator/edit", verifyToken, (req,res)=>{
  sql = "select c.id as coordinatorId, c.* from coordinator c join faculty f on c.associatedDepartment = f.associatedDepartment where f.facultyEmail=?";
  con.query(sql, [req.faculty.facultyEmail],(err, result)=>{
    if(err) throw err;
    var coordinatorId = [];
    ...
    res.render("editCoordinator.ejs", {coordinatorId:coordinatorId, coordinatorEmail:coordinatorEmail, associatedEvent:associatedEvent});
  });
});
...
router.post("/manage_coordinator/edit/update", verifyToken, (req,res)=>{
  ...
  con.query(sql, [coordinatorId], (err, result)=>{
    ...
    res.render("updateCoordinator.ejs",{coordinatorId:coordinatorId, coordinatorEmail:coordinatorEmail, associatedEvent:associatedEvent});
  });
});

router.post("/manage_event/edit", verifyToken, (req,res)=>{
  sql = "select e.id as eventId,e.* from event e join faculty f on e.department = f.associatedDepartment where f.facultyEmail=?";
  con.query(sql, [req.faculty.facultyEmail],(err, result)=>{
    if(err) throw err;
    var eventId = [];
    var eventName = [];
    ...
    res.render("editEvent2.ejs", {eventId:eventId, eventName:eventName, eventTagline:eventTagline, eventDescription:eventDescription, eventVenue:eventVenue, numberOfCoordinators:numberOfCoordinators, eventIcon:eventIcon, eventFlyer:eventFlyer, eventDate:eventDate, eventStartTime:eventStartTime, eventEndTime:eventEndTime, registrationFees:registrationFees, attendeesCapacity:attendeesCapacity});
  });
});

router.post("/manage_coordinator/edit/updatecoordinator", verifyToken, (req,res)=>{
  var coordinatorId = req.body.coordinatorId;
  ...
  sql = "update coordinator set coordinatorEmail=?,coordinatorPassword=? where id=?";
  con.query(sql, [coordinatorEmail, hashPassword, coordinatorId], (err, result)=>{
    if(err) throw err;

    console.log("coordinator's data updated successfully");
    send_mail2("Coordinator of "+associatedEvent, coordinatorPassword, "Your password is: ", coordinatorEmail);
...
router.post("/manage_event/edit/update", verifyToken, (req,res)=>{
  ...
  con.query(sql, [eventId], (err, result)=>{
    ...
    res.render("updateEvent2.ejs", {eventId:eventId, eventName:eventName, eventTagline:eventTagline, eventDescription:eventDescription, eventVenue:eventVenue, numberOfCoordinators:numberOfCoordinators, eventIcon:eventIcon, eventFlyer:eventFlyer, eventDate:eventDate, registrationFees:registrationFees, attendeesCapacity:attendeesCapacity, hours1:hours1, minutes1:minutes1, period1:period1, hours2:hours2, minutes2:minutes2, period2:period2});
    });
});
          ...
          file2.mv(destinationPath2, (err)=>{
              if(err){
                res.send(err);
                  ...
                  var sql = "update event set eventName=?, eventTagline=?, eventDescription=?, eventVenue=?, numberOfCoordinators=?, eventIcon=?, eventFlyer=?, eventDate=?, eventStartTime=?, eventEndTime=?, registrationFees=?, attendeesCapacity=?, department=?, facultyEmailId=? where id=?";
                  con.query(sql,[req.body.eventName, req.body.eventTagline, req.body.eventDescription, req.body.eventVenue, req.body.numberOfCoordinators, filename1, filename2, formattedDate, eventStartTime, eventEndTime, req.body.registrationFees, req.body.attendeesCapacity, department, faculty_email, eventId],(err,result)=>{
                    if(err) throw err;
