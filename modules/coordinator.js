...
router.post("/", (req,res)=>{
  ...
  var sql = "select distinct ur.*, e.eventName from user_registration ur join registered_event re on ur.user_id = re.user_id join event e on re.event_id = e.id join coordinator c on e.eventName = c.associatedEvent where c.coordinatorEmail=?";
  con.query(sql, [email_id],(err, result)=>{
    if(err) throw err;
      ...
      con.query(sql, [req.session.email_id], (err, result)=>{
        ...
        res.render("coordinatorDashboard.ejs", {eventsName: eventsName, userOriginalID: userOriginalID, userName: userName, userEmail: userEmail, userPhone: userPhone, userGender: userGender, userDOB: userDOB, userDepartment: userDepartment, userYear: userYear, userEnroll: userEnroll});
      });
  });
});
...
router.post("/login",(req,res)=>{
    ...
    var password = req.body.password;
        ...
        con.query(sql,[email],(err,result)=>{
            if (err) throw err;
            if(result.length == 0){
            res.render("coordinatorLogin.ejs",{emailId:"Email Id not exists!", existEmailId:email, existPassword:password});
            }
            else{

                coordinator_email = result[0].coordinatorEmail;
            bcrypt.compare(password, result[0].coordinatorPassword, (err,passwordMatch)=>{
                ...
                {
                    res.render("coordinatorLogin.ejs",{password:"Password is incorrect!",existEmailId:email, existPassword:password});
                }
                }
            });
            }
            ...
            con.query(sql,[email],(err,result)=>{
                ...
                if(result.length == 0){
                  ...
                  con.query(sql,[email],function(err,result){
                      if (err) throw err;
                      res.render("coordinatorSendOTP.ejs",{randomOtp : result[0].otp, millisecs : result[0].created_at, emailId : result[0].email, forgotpassword : "forgotpassword"});
                  });
                }
              });
    });
